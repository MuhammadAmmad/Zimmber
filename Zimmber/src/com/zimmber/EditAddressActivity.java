...
import com.zimmber.asynctask.EditAddressAsynctask;
...
public class EditAddressActivity extends Activity implements
	...
	String email = "", access_token = "", address_id = "",
			select_address_id = "", address_title = "", state_id = "",
			city_id = "", location_id = "", landmark = "", street = "",
			flat_no = "", address = "", pincode = "";

	String get_state_id = "", get_city_id = "", get_location_id = "";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		get_state_id = getIntent().getStringExtra("state_id");
		get_city_id = getIntent().getStringExtra("city_id");
		get_location_id = getIntent().getStringExtra("location_id");

		if (getIntent().getStringExtra("address_title").toString().length() > 0) {

			et_address_title.setText(getIntent()
					.getStringExtra("address_title"));
		...
		et_street.setText(getIntent().getStringExtra("street"));
		et_flatno.setText(getIntent().getStringExtra("flat_no"));
		et_address.setText(getIntent().getStringExtra("address"));
		et_pincode.setText(getIntent().getStringExtra("pincode"));
	...
	protected void onStart() {
		// TODO Auto-generated method stub
		super.onStart();
		GoogleAnalytics.getInstance(EditAddressActivity.this)
				.reportActivityStart(EditAddressActivity.this);
	...
	protected void onStop() {
		...
		super.onStop();
		GoogleAnalytics.getInstance(EditAddressActivity.this)
				.reportActivityStop(EditAddressActivity.this);
	}
	...
	private void initialize() {
		...
		sharedpreference = new SharedPreferenceClass(EditAddressActivity.this);

	}

	public boolean onKeyDown(int keyCode, KeyEvent keyEvent) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			Intent i = new Intent(EditAddressActivity.this,
	...
	private void onclick() {
		...
		spin_title.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view,
				...
				select_address_id = _titleAdapter.getItem(position).getStrId();
				address_title = _titleAdapter.getItem(position).getName();

				if (select_address_id.equals("0")) {

					address_title = "";
				}

				if (select_address_id.equals("5")) {
		...
		btn_save_address.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (address_title.equals("")) {
					...
					alert_dialog = new Dialog(EditAddressActivity.this);
					...
					if (Utils.checkConnectivity(EditAddressActivity.this)) {

						EditAddressAsynctask edit_address = new EditAddressAsynctask(
								EditAddressActivity.this);
						edit_address.editaddressintf = EditAddressActivity.this;
						edit_address.execute(email, access_token, state_id,
								city_id, location_id, landmark, street,
								flat_no, address, pincode, address_id,
								address_title);
	...
	private void loadLocation() {
		...
		_titleAdapter = new DropDownAdapter(EditAddressActivity.this,
				_titleItems);
		spin_title.setAdapter(_titleAdapter);

		_stateItems = new ArrayList<DropDownItem>();
		_stateAdapter = new DropDownAdapter(EditAddressActivity.this,
				_stateItems);
		spin_state.setAdapter(_stateAdapter);

		_cityItems = new ArrayList<DropDownItem>();
		_cityAdapter = new DropDownAdapter(EditAddressActivity.this, _cityItems);
		spin_city.setAdapter(_cityAdapter);

		_locationItems = new ArrayList<DropDownItem>();
		_locationAdapter = new DropDownAdapter(EditAddressActivity.this,
				_locationItems);
			...
			if (result != null) {
				try {
					...
					for (int state_pos = 0; state_pos < _stateItems.size(); state_pos++) {
						if (_stateItems.get(state_pos).getStrId()
								.equals(get_state_id)) {
				...
				try {
					...
					if (city_flag == 1) {
						...
						for (int city_pos = 0; city_pos < _cityItems.size(); city_pos++) {
							...
							if (_cityItems.get(city_pos).getStrId()
									.equals(get_city_id)) {
				...
				try {
					...
					if (location_flag == 1 || location_flag == 2) {
						...
						for (int location_pos = 0; location_pos < _locationItems
								.size(); location_pos++) {
							if (_locationItems.get(location_pos).getStrId()
									.equals(get_location_id)) {
	...
	@Override
	public void onStartedEditAddress() {
		// TODO Auto-generated method stub

		pDialog = new ProgressDialog(EditAddressActivity.this);
	...
	@Override
	public void onCompletedEditAddress(String errorcode, String message) {
		// TODO Auto-generated method stub
		...
		if (errorcode.equals("0")) {
			...
			alert_msg.setText("Address Edited Successfully");
